...
// Zabezpieczenie musi być PRZED próbą połączenia z bazą
if (!connectionString) {
...
// LOGOWANIE RODZINY (Nowy endpoint)
app.post('/api/family-access', async (req, res) => {
  ...
  try {
    ...
    if (!patient) {
      return res.status(401).json({ error: "Nieprawidłowy numer PESEL lub token dostępu." });
    }

    res.json(patient);
  } catch (error: unknown) {
    console.error("Błąd autoryzacji rodziny:", error);
    res.status(500).json({ error: "Wystąpił błąd serwera podczas weryfikacji danych." });
  }
});
